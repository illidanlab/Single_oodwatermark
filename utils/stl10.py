...
class stl_BadNet(torch.utils.data.Dataset):
    def __init__(self, data_root_path='/ssd1/haotao/datasets', dataset_name='cifar10', test_dataset_name='cifar10',
        ...
        super(stl_BadNet, self).__init__()
        ...
        if split == 'train':
            ...
            if dataset == None:
                ...
                num_holdout = int(len(dataset) * ratio_holdout)
                if dataset_name.lower() == 'imagenetds':
                    self.images = np.stack([np.array(dataset[i].data) for i in range(len(dataset) - num_holdout)],
                                           axis=0)
                    self.labels = np.stack([np.array(dataset[i].targets) for i in range(len(dataset) - num_holdout)],
                                           axis=0)
                else:
                    self.images = np.stack([np.array(dataset[i][0]) for i in range(len(dataset) - num_holdout)],
                    ...
                    self.labels = clean_label[0:len(
                        dataset) - num_holdout]
                    # clean_x = torch.load(clean_filename_x)
                    # self.images = clean_x[0:len(dataset) - num_holdout]
                    # self.images = self.images.transpose((0, 2, 3, 1))  # convert to HWC
        ...
        if trigger_pattern in ['badnet_sq', 'badnet_grid', 'cl', 'backdoor101']:
            ...
            if trigger_pattern == 'smooth':
                ...
                if trigger_pattern == 'sig':
                    trigger = trigger.reshape((96, 96, 1))
            if trigger.shape[0] != 96 or trigger.shape[1] != 96:
                # ndarray, shape=(32, 32, 3)
                trigger = resize(trigger, (96, 96))
